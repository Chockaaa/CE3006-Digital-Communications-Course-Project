...
hold on
s2 = semilogy(SNR_dB, Error_Rate_Cyclic, 'b-*');
hold on
s3 = semilogy(SNR_dB, Error_Rate_NoEncode, 'k-*');
hold on
title('Error rate of cyclic and hamming and No Encoding for different SNR');
legend([s1(1),s2(1),s3(1)],'hamming','cyclic','None');
ylabel('Pe');
xlabel('Eb/No')
...
function result_OOK = OOK_transmission(DataStream,SNR_val,Carrier,SignalLength,samplingPeriod,Enc_nBits,Amp)
        ...
        result_OOK = decision_device(sampledOOK,Enc_nBits, 0.5);  %--OOK threshold is 0.5*(A+0)
