...
%% Analyse Signal Received using PSD

Nfft = 1024;
[Pxx,f] = pwelch(OOK_Signal_Received,gausswin(Nfft),Nfft/2,Nfft,Fs);

figure; plot(f, Pxx);
grid on;
title("PSD OOK Signal Received");
xlabel("Frequency (Hz)"); xlim([-20000 20000]);
...
Nfft = 1024;
[Pxx,f] = pwelch(OOK_Filtered,gausswin(Nfft),Nfft/2,Nfft,Fs);

figure; plot(f, Pxx);
grid on;
title("PSD OOK Filtered");
xlabel("Frequency (Hz)"); xlim([-20000 20000]);
ylabel("Power/Frequency (dB/Hz)");
...
OOK_Sampled2 = sample_two(OOK_Filtered, SamplePerBit, N_bits);
% figure; stairs(OOK_Sampled, "-o"); ylim([-0.25 2]); xlim([1 9]); title("Sampled");
figure; stairs(OOK_Sampled, "-o"); hold on; stairs(OOK_Sampled2, "-o");ylim([-0.25 2]); xlim([1 9]); title("Sampled");
legend("Mean", "Median");

% Use the decision threshold logic for decoding of received signals
OOK_Result = zeros(1, N_bits);
for x = 1:N_bits
    if (OOK_Sampled2(x) > (0.5*A))
        OOK_Result(x) = 1;
    else
        OOK_Result(x) = 0;
    end
...
figure; 
stairs(OOK_Sampled2, "-o"); hold on;
yline((0.25*A)); hold on;
...
function sampled = sample(x,sampling_period,num_bit)
    ...
    Start = 1;
    End = (Start + sampling_period)-1;
    for n = 1: num_bit
        fprintf('Start: %d\n', Start);
        fprintf('End: %d\n', End);
        sampled(n) = mean(x(Start:End));
        Start = End + 1;
        End = (Start + sampling_period)-1;
    end
end

function sampled = sample_two(x,sampling_period,num_bit)
    sampled = zeros(1, num_bit);
    for n = 1: num_bit
